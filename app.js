...
async function openTicketDetail(ticketId) {
  ...
  try {
    ...
    const voiceHtml = voices.map(voice => `<div class="ticket-timeline-item"><strong>Llamada ${escapeHtml(voice.status || "")}</strong><div>${escapeHtml(voice.requested_by || "")} → ${escapeHtml(voice.target_type || "")}${voice.duration_seconds ? ` · ${voice.duration_seconds}s` : ""}</div><small>Sesión WA-Center: ${escapeHtml(voice.wa_center_session_id || "no informada")}${voice.recording_id ? ` · Grabación: ${escapeHtml(voice.recording_id)}` : ""}</small>${voice.recording_url ? `<audio class="ticket-media" controls playsinline preload="metadata" src="${escapeHtml(voice.recording_url)}"></audio>` : '<small>Sin grabación disponible</small>'}</div>`).join("");
